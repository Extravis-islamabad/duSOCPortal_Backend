...
from tenant.models import (
    ...
    IBMQraderAllowedOutbounds, IBMQraderAllowedInbounds,
)
...
class IBMQradar:
    ...
    def _transform_allowed_inbounds_data(self, data_list):
        ...
        df["destination_country_and_region_count"] = df["Destination Geographic Country/Region (Unique Count)"].fillna(
            "").astype(str)
        df["count"] = df["Count"].fillna(0).astype(int).astype(str)
        df["total_event_count"] = df["Event Count (Sum)"].fillna(0).astype(int).astype(str)
    ...
    def _insert_allowed_inbounds_data(self, data):
        ...
        try:
            ...
            logger.info(f"_insert_allowed_inbounds_data() took: {time.time() - start:.2f} seconds")
        except Exception as e:
