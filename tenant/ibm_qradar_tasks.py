...
@shared_task
def sync_qradar_token_tenants(
    ip_address: str, port: int, api_key: str, integration_id: int
):
    """
    Syncs the QRadar tenants with the database using an API key.

    This task fetches the domains (tenants) from the IBM QRadar instance using an API key and
    inserts them into the database.

    :param ip_address: The IP address of the QRadar instance.
    :param port: The port to use when connecting to the QRadar instance.
    :param api_key: The API key to use when connecting to the QRadar instance.
    :param integration_id: The ID of the integration for which to sync the tenants.
    """
    start = time.time()
    logger.info("Running QRadarTasks.sync_qradar_tenants() task")
...
@shared_task
def sync_event_collectors_token(
    api_key: str, ip_address: str, port: str, integration_id: int
):
    """
    Syncs the event collectors for a given QRadar integration using an API key.
    ...
    try:
        with IBMQradar(
            ...
            if not flag:
                ...
                return
            data = ibm_qradar._get_eps_results_by_search_id(search_id=search_id)
            transformed_data = ibm_qradar._transform_allowed_inbounds_data(data)
            if transformed_data:
...
@shared_task
def sync_event_log_assets_token(
    api_key: str, ip_address: str, port: str, integration_id: int
):
    """
    Syncs the event log assets for a given QRadar integration using an API key.

    This task fetches the event log assets from the IBM QRadar instance using an API key and
    inserts them into the database.

    :param api_key: The API key to use when connecting to the QRadar instance.
    :param ip_address: The IP address of the QRadar instance.
    :param port: The port to use when connecting to the QRadar instance.
    :param integration_id: The ID of the integration for which to sync the event log assets.
    """
    start = time.time()
    logger.info("Running QRadarTasks.sync_event_log_assets() task")
    try:
...
@shared_task
def sync_event_log_assets_groups_token(
    api_key: str, ip_address: str, port: str, integration_id: int
):
    start = time.time()
    logger.info("Running QRadarTasks.sync_event_log_assets_groups() task")
...
@shared_task
def sync_event_log_sources_types_token():
    results = IntegrationCredentials.objects.filter(
        integration__integration_type=IntegrationTypes.SIEM_INTEGRATION,
        integration__siem_subtype=SiemSubTypes.IBM_QRADAR,
        credential_type=CredentialTypes.API_KEY,
...
@shared_task
def sync_ibm_qradar_data_token():
    results = IntegrationCredentials.objects.filter(
        integration__integration_type=IntegrationTypes.SIEM_INTEGRATION,
        integration__siem_subtype=SiemSubTypes.IBM_QRADAR,
        credential_type=CredentialTypes.API_KEY,
    ...
    for result in results:
        sync_qradar_token_tenants.delay(
            api_key=result.api_key,
            ip_address=result.ip_address,
            port=result.port,
            integration_id=result.integration.id,
        )
        sync_event_collectors_token.delay(
            api_key=result.api_key,
            ip_address=result.ip_address,
            port=result.port,
            integration_id=result.integration.id,
        )
        sync_event_log_assets_groups_token.delay(
            api_key=result.api_key,
            ip_address=result.ip_address,
            port=result.port,
            integration_id=result.integration.id,
